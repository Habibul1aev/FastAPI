...
class ProductService:
    ...
    def get_product_by_id(self, product_id: int) -> ProductResponse:
        ...
        if not product:
            raise HTTPException(status=status.HTTP_404_NOT_FOUND, detail=f'Product with id {product_id} not found')
    ...
    def get_products_by_category(self, category_id: int) -> ProductListResponse:
        ...
        if not category:
            raise HTTPException(status=status.HTTP_404_NOT_FOUND, detail=f'Category with id {category_id} not found')

        product = self.product_repository.get_by_category(category_id)
    ...
    def create_product(self, product_data: ProductCreate) -> ProductResponse:
        ...
        if not category:
            raise HTTPException(status=status.HTTP_400_BAD_REQUEST, detail=f'Category with id {product_data.category_id} not found')
